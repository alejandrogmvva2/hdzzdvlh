...
const handleChatConnection = (channel, socket) => {
  ...
  chatClient.connect().then(() => {
    setTimeout(() => {
      chatClient.say(channel, "Song Request esta activado en este canal.");
    }, 2500);
  });
  ...
  if (
    ...
    getVideoDetails(videoURL)
      .then((youtubeData) => {
        ...
        chatClient.say(
          channel,
          `@${user}: Canción añadida a la cola.`
        );
      })
      .catch((err) => {
        chatClient.say(channel, `@${user}: No encuentro esa canción.`);
      });
  }
};
